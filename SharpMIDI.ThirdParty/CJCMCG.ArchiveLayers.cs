    ...
    class CJCMCG
    {
        ...
        public static Stream AddXZLayer(Stream input)
        {
            try
            {
                ...
                Console.WriteLine("Added xz layer with xz.exe");
                return xz.StandardOutput.BaseStream;
            }
            catch (Exception)
            {
                MessageBox.Show("Falling back to SharpCompress xz loader, this is slower and may break!");
                Console.WriteLine("Added xz layer with SharpCompress");
        ...
        public static Stream AddZipLayer(Stream input)
        {
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                {
                    filein = entry.Name;
                    Console.WriteLine("Added zip layer with SharpCompress");
                    return entry.Open();
                }
            }
        ...
        public static Stream AddRarLayer(Stream input)
        {
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                {
                    filein = entry.Key;
                    Console.WriteLine("Added rar layer with SharpCompress");
        ...
        public static Stream Add7zLayer(Stream input)
        {
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                {
                    filein = entry.Key;
                    Console.WriteLine("Added 7-zip layer with SharpCompress");
                    return entry.OpenEntryStream();
                }
            }
        ...
        public static Stream AddTarLayer(Stream input)
        {
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                {
                    filein = entry.Key;
                    Console.WriteLine("Added tar layer with SharpCompress");
                    return entry.OpenEntryStream();
                }
            }
        ...
        public static Stream AddGZLayer(Stream input)
        {
            ...
            foreach (var entry in zip.Entries)
            {
                ...
                {
                    filein = entry.Key;
                    Console.WriteLine("Added gzip layer with SharpCompress");
                    return entry.OpenEntryStream();
                }
            }
